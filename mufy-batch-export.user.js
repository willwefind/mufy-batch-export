...
(function () {
  ...
  // 优先级：真的什么都没查出来 > 卡明确没了 > 卡打不开 > 记录状态。
  // 🔴 「卡打不开」和「作者删了卡」必须分开：2026-08-25 实测它们的状态码、
  //    是否聊过、名字长相三项全都不同（见本节开头）。合成一栏就等于把两件事说成一件。
  const bucketOf = (r) => (r.status === 'error' ? 'error'
    : r.cardGone ? 'gone'
    : r.cardUnread ? 'locked'
    : r.status);
  ...
  const CHECK_ADVICE = {
    gone: [
      '接口明确答复「没有这张卡」（HTTP 500/404，连问三次都一样，不是网络问题）。开场白拿不到了。',
      '⚠️ **名字显示成「已下架角色」是 mufy 那边给的**，不是脚本坏了 ——',
      '   卡一下架，它连角色列表里的名字都一并换掉了，真名再也读不到。',
      '**但你的聊天记录多半还在**，照常导得出来：范围选「全部聊过的角色」，',
      '或者「手动填角色 ID」把下面这些 ID 一个个补导。',
      '（实测这一批**全都是你聊过的**角色 —— 聊过才会留在列表里被你看见。）',
      '⚠️ 下面标了「连记录也不剩了」的那几个除外 —— 那种卡和记录一起没，导也导不出内容。',
    ],
    // 🔑 这一栏就是 clover 要的那份名单，她原话：「目前收藏里已经打不开的卡」
    locked: [
      '这些角色的卡**现在打不开**：接口一直回 HTTP 503，**连问三次都一样，不是网络抖动**。',
      '实测（2026-08-25，一个 302 个收藏的账号，抽 120 个）：503 的这一批',
      '**全都是「收藏了但从没聊过」的**；而「已下架」的那种给的是 500、名字会变成「已下架角色」。',
      '两者不是一回事，所以分成了两栏。',
      '⚠️ **卡打不开 ≠ 你的记录没了。** 下面每一行都写着它自己的记录状态，有记录的照常导得出来。',
      '想确认它到底还在不在：用下面「上面那些角色ID怎么用」那一节，拿 ID 直接去开它的页面看看。',
    ],
    empty: [
    ...
    error: [
      '这一栏是「**连记录状态都没查出来**」—— 存档或最后那段的接口这次失败了。',
      '（卡读不到已经不算在这里了：卡和记录是两套接口，卡打不开时记录照样查得动，',
      '  那种会按记录状态归到别的栏，并在行里标一句「卡打不开」。）',
  ...
  async function checkOne(c) {
    ...
    const row = {
      ...
      cardGone: false,
      cardUnread: false,     // 卡这次读不到（503 之类），但记录照样查得动
      cardErr: '',
      httpStatus: 0,
      archives: 0,
    ...
    try {
      ...
      if (/HTTP (404|500)|not found/i.test(msg)) {
        // 实测：卡下架/被删之后这个接口就是 500，而且**连问三次都一样**。
        // 它还在你的列表里 ⇒ 是下架/删卡，不是设私密（设私密会被整个摘出列表）。
        row.cardGone = true;
      } else {
        // 503 / 403 / 网络错误 —— 只能说「这张卡这次读不到」，不能说卡没了
        row.cardUnread = true;
      }
      // 🔴🔴 这里**绝对不能 return**。
      //    2026-08-25 实测：卡读不到的时候，query_archives 和 dialogs/query 照样 200 能用 ——
      //    「卡」和「记录」是两套接口，卡打不开不代表记录查不出来。
      //    v1.51 及以前一读卡失败就 return，于是 503 的那一批（实测全是「收藏了但从没聊过」的）
      //    统统被丢进「❓ 这次没查出来」，而它们真正的状态是「📭 从没聊过」——
      //    clover 一眼就看出不对：「其实可能不是网络波动，而是收藏了但是没有聊天」。
      //    她说对了，而错的是**判定顺序**，不是判据本身。
    }
    if (card) {
      if (card.name) row.name = card.name;
      if (!row.lastSessionId && card.lastSessionId) row.lastSessionId = card.lastSessionId;
    }
    // 卡读不到时，名字就用列表里那个（row.name 初始化时already 是它）——
    // ⚠️ 下架的卡，mufy 连列表里的名字都换成了「已下架角色」，那是它给的，不是这里退化的。
    ...
    // 卡的状态单独成一句，和记录状态拼在一起 —— 两件事都要说
    const 卡注 = row.cardGone ? '卡已下架/被删；'
      : row.cardUnread ? `卡这次打不开（${row.httpStatus ? 'HTTP ' + row.httpStatus : row.cardErr}）；` : '';

    if (row.archives > 0) {
      row.status = 'ok';
      row.detail = 卡注 + `${row.archives} 段存档`;
      return row;
    }
    ...
    if (!row.lastSessionId) {
      if (row.chatted) {
        ...
        row.status = 'empty';
        row.detail = 卡注 + '在「聊过的角色」列表里，却没有存档、也没有「最后聊的那一段」。';
      } else {
        row.status = 'never';
        row.detail = 卡注 + '没有存档，也没有「最后聊的那一段」——从没聊过。';
      }
      return row;
    }
    try {
    ...
    if (row.liveHasMessages) {
      row.status = 'ok';
      row.detail = 卡注 + '没有存档，但「最后聊的那一段」还在（这种段只有本脚本导得出来）。';
    } else {
      row.status = 'empty';
      row.detail = 卡注 + '存档 0 段，「最后聊的那一段」也一条不剩。';
    }
    return row;
  }
  ...
  function buildCheckReport(rows, meta) {
    ...
    for (const [key, icon, label] of CHECK_BUCKETS) {
      ...
      // 「卡打不开」这一栏里，每一行的**记录状态**才是用户真正要的信息，
      // 所以这里按记录状态再点一次数 —— 不然一栏几十个，看不出哪些其实还有东西。
      if (key === 'locked') {
        const cnt = {};
        for (const r of list) cnt[r.status] = (cnt[r.status] || 0) + 1;
        const 说 = { ok: '记录还在', empty: '一条记录都不剩', never: '从没聊过', error: '记录也没查出来' };
        const 摘要 = Object.keys(说).filter((k) => cnt[k]).map((k) => `${说[k]} ${cnt[k]} 个`).join('，');
        if (摘要) { L.push(''); L.push(`  这一栏按记录状态分：${摘要}。`); }
      }
      ...
      for (const r of list) {
        ...
        // 卡没了 / 打不开的两栏里都要再分一次：记录也空的那几个，「记录还在」对它们是错的
        if ((key === 'gone' || key === 'locked') && r.status === 'empty') {
          L.push('      ⚠️ 连记录也不剩了。');
        }
        if (key === 'locked' && r.status === 'never') L.push('      （你从没聊过它，所以本来也没有记录。）');
        if (key === 'locked' && r.status === 'ok') L.push('      🔑 卡虽然打不开，但记录还在，照常导得出来。');
    ...
    L.push('── 这份报告是怎么判的 ──');
    L.push('  🔑 先分清两件事：**卡**（角色本身还在不在）和**记录**（你和它聊的内容还在不在）。');
    L.push('     它们走的是两套接口，**卡打不开不代表记录查不出来** —— 所以下面每一行');
    L.push('     都会同时告诉你这两件事。');
    L.push('');
    L.push('  ✅ 正常　　＝ 卡在，且（有存档 或 「最后聊的那一段」里还有内容）');
    L.push('  🔒 已下架　＝ 接口**明确答复「没有这张卡」**（HTTP 500/404，连问三次都一样）。');
    L.push('               名字会显示成「已下架角色」—— 那是 mufy 给的，卡一下架');
    L.push('               连列表里的名字都被换掉了。实测这批**全都是你聊过的**角色。');
    L.push('               ⚠️ 设为私密的不在这里 —— 那种会被整个摘出列表，压根不会出现在报告里。');
    L.push('  🚫 打不开　＝ 接口一直回 **HTTP 503**（连问三次都一样，**不是网络抖动**）。');
    L.push('               实测这批**全都是「收藏了但从没聊过」的**，和 🔒 那种不是一回事。');
    L.push('               卡打不开，但记录照样查得出来，所以行里会写它自己的记录状态。');
    L.push('  💬 一条不剩＝ 卡在，但存档 0 段、「最后聊的那一段」也 0 条。');
    L.push('               卡还在就还有得救：去收藏或随手聊一条，入口能保住。');
    L.push('  📭 从没聊过＝ 没有存档，也没有「最后聊的那一段」。**这批是正常的**，');
    L.push('               公开卡从收藏里照样点得进去，只是你还没开聊。');
    L.push('  ❓ 没查出来＝ **连记录状态都没查出来**（存档或最后那段的接口这次失败了）。');
    L.push('               卡读不到已经不算在这里 —— 那种归 🔒 或 🚫。');
    L.push('');
