...
(function () {
  ...
  async function getDialogs(sessionId, characterId) {
    ...
    //    ⚠️⚠️ **已经在唯一的真实样本上证伪了，别再抱期待，也别再想别的翻法。**
    //    2026-08-10，一位用户那段「官方导出 2971 条、接口只给 1000」的对话上，
    //    四种翻法**全部停在同一个 1000**：
    //      · pageSize=500 × 2 页        → 1000
    //      · pageSize=100 × 10 页       → 1000
    //      · 从第 3 页往后要（v1.33）    → 一条不给
    //      · 一次请求要 2000 条（这一发）→ 一条不给
    //    四条曲线交在同一个点 ⇒ **卡的是偏移量本身**（服务端只让你看到最近 1000 条），
    //    不是页大小、也不是翻页深度。这一段更早的内容，网页接口这条路拿不到。
    //    → 用户侧的出路写在 README 里：**mufy 官方的引继码导出**（另一条路）能拿到全史。
    //    这一发留着不删：代价只有一次请求、只落在可疑段上，
    //    而我们只验过一个账号的一段，不排除别处行为不同。**但别把它当成有效。**
